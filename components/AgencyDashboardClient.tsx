            ...
            <div className="relative z-50 w-full max-w-4xl max-h-[90vh] overflow-hidden rounded-lg bg-white shadow-xl">
              ...
              <div className="overflow-y-auto px-6 py-6 max-h-[calc(90vh-140px)]">
                <div className="space-y-6">
                  ...
                  {/* Travel Dates */}
                  {(selectedSubmission.travel_months || selectedSubmission.specific_date) && (
                    <div className="border-t border-gray-200 pt-6">
                      <h4 className="text-sm font-medium text-gray-500 uppercase tracking-wide mb-3">
                        Travel Dates
                      </h4>
                      <dl className="space-y-3">
                        {selectedSubmission.travel_months && selectedSubmission.travel_months.length > 0 && (
                          <div>
                            <dt className="text-sm font-medium text-gray-700">Preferred Months</dt>
                            <dd className="mt-1 text-sm text-gray-900">{selectedSubmission.travel_months.join(', ')}</dd>
                          </div>
                        )}
                        {selectedSubmission.specific_date && (
                          <div>
                            <dt className="text-sm font-medium text-gray-700">Specific Date</dt>
                            <dd className="mt-1 text-sm text-gray-900">
                              {new Date(selectedSubmission.specific_date).toLocaleDateString('en-US', {
                                year: 'numeric',
                                month: 'long',
                                day: 'numeric',
                              })}
                  ...
                  {/* Mode-Specific Data */}
                  {selectedSubmission.mode_specific_data && Object.keys(selectedSubmission.mode_specific_data).length > 0 && (
                    <div className="border-t border-gray-200 pt-6">
                      <h4 className="text-sm font-medium text-gray-500 uppercase tracking-wide mb-3">
                        Additional Details
                      </h4>
                      <div className="bg-gray-50 rounded-md p-4">
                        <dl className="space-y-4">
                          {Object.entries(selectedSubmission.mode_specific_data).map(([key, value]) => {
                            const formattedKey = key
                              .replace(/([A-Z])/g, ' $1')
                              .replace(/^./, (str) => str.toUpperCase())
                              .trim();
