...
export const GolfFocus = ({ data, updateData }: GolfFocusProps) => {
  ...
  const toggleGolf = (checked: boolean) => {
    updateData({
      ...
      golfRounds: checked ? 1 : 0,
    });
  };

  const increment = () => {
    updateData({
      golfRounds: Math.min((data.golfRounds || 1) + 1, 10),
    });
  };

  const decrement = () => {
    updateData({
      golfRounds: Math.max((data.golfRounds || 1) - 1, 1),
    });
  };
    ...
    >
      <div className="max-w-sm mx-auto">
        ...
        <div
          ...
          `}
        >
          {/* LOTTIE (always visible, animates on toggle) */}
          ...
          {/* ROUNDS – APPEAR ONLY AFTER TOGGLE */}
          <div
            className={`
              mt-3 pt-3 border-t
